...
def preprocess(df_raw):
    ...
    df_raw["datetime"] = pd.to_datetime(df_raw["date"].astype(str) + " " + df_raw["time"], errors="coerce")
...
def add_features(df, mode="train"):
    ...
    df["rolling_mean_prev_day"] = (
        df.groupby("group_key")["ridership_4h"]
        .transform(lambda x: x.shift(6).rolling(6).mean())
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inference_table", required=False, help="Table to save inference features")

    args = parser.parse_args()

    df_raw = load_raw_data(args.user, args.password, args.host, args.port, args.db_name, args.raw_table)
    df_agg = preprocess(df_raw)
    df_features = add_features(df_agg, mode=args.mode)

    if args.mode == "inference" and args.inference_table:
        save_features(df_features, args.user, args.password, args.host, args.port, args.db_name, args.inference_table)
    else:
        save_features(df_features, args.user, args.password, args.host, args.port, args.db_name, args.features_table)
