...
def fetch_actual_data(date: str) -> pd.DataFrame:
    ...
    if response.status_code != 200:
        raise Exception(f"Failed to fetch data for {date}: {response.status_code}")
    ...
    df = df.dropna(subset=["datetime", "entries", "exits", "c_a", "unit", "scp"])
...
def load_predictions(user, password, host, port, db_name, predictions_table, target_date):
...
def save_monitoring_data(df, df_global, user, password, host, port, db_name, monitoring_table):
    engine = create_engine(f"postgresql://{user}:{password}@{host}:{port}/{db_name}")
    df.to_sql(name=monitoring_table, con=engine, if_exists="append", index=False)

    df_global.to_sql(name=monitoring_table+'_global', con=engine, if_exists="append", index=False)



    print(f"[OK] Saved {len(df)} rows to '{monitoring_table}'")

def run_monitoring(user, password, host, port, db_name, predictions_table, monitoring_table, reference_date):
    reference_date = datetime.strptime(args.reference_date, "%Y-%m-%d").date()
    print(f"[Monitoring] For date: {reference_date}")

    df_pred = load_predictions(user, password, host, port, db_name, predictions_table, reference_date)
    df_actual = fetch_actual_data(str(reference_date))
    print(reference_date)

    df = pd.merge(df_pred, df_actual, on=["datetime", "group_key"], how="inner")

    df["abs_error"] = (df["actual_ridership"] - df["predicted_ridership"]).abs()
    df["squared_error"] = (df["actual_ridership"] - df["predicted_ridership"]) ** 2
    ...
    df_metrics["load_class"] = pd.qcut(df_metrics["ridership_mean"], q=4, labels=["low", "mid-low", "mid-high", "high"])
    ...
    save_monitoring_data(df_metrics, df_global_metrics, user, password, host, port, db_name, monitoring_table)
...
if __name__ == "__main__":
    ...
    run_monitoring(
        ...
        args.db_name, args.predictions_table, args.monitoring_table, args.reference_date 
    )
